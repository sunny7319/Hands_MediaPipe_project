...
# recognition_lib의 MLmodel.py 파일에서 onehand_model 함수 수정
def onehand_model(landmark):
    data = get_data(landmark)    
    model = keras.models.load_model('game1/ML/one_DNN_stop.h5')

    y_prob = predict_with_model(model, data)
...
def twohand_model(landmark1, landmark2):
    ...
    model = keras.models.load_model('game1/ML/two_DNN_duck_flower2.h5')
